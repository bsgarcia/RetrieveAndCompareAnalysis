...
models = {'RW', 'RW_{\lambda}'};

paramlabels = {
    {'\beta', '\alpha'},...
    {'\beta', '\alpha', '\lambda_{D}', 'lambda_{E}'}};
...
for exp_num = selected_exp
    ...
    nmodel = length(learning_model);
    nsub = d.(exp_name).nsub;
    nparam = length(paramlabels);
    
    init_value = {[0, .5], [0, .5, .5]};
    lb = {[0, 0], [0, 0, 0]};
    ub = {[100, 1], [100, 1, 1]};
    
    try
        data = load(sprintf('%s%s_learning_%d',...
            fit_folder, fit_filename, sess));
...
%         
%     Q = get_qvalues({beta1, alpha1}, cho, cfcho, con, out, cfout, size(con, 2), 1);
%     
%     figure('Position', [1,1,900,600]);
% 
%     plot_Q(Q, p1, p2, blue_color, exp_num);
    ...
    Q = get_qvalues({beta1, alpha1, alpha2}, cho, cfcho, con, out, cfout, size(con, 2), 2);
...
function [parameters,ll] = ...
    runfit_learning(nsub, whichmodel, con, cho, cfcho, out,...
    cfout, ntrials, init_value, lb, ub, folder, fit_filename, sess)
    ...
    data = containers.Map({'parameters', 'll'},...
        {parameters, ll});
    save(sprintf('%s%s_learning_%d', folder, fit_filename, sess), 'data');
     close(w);
%     
end
...
function plot_Q(qvalues, ev1, ev2, blue_color, exp_num)
    ...
    saveas(gcf, sprintf('fig/exp/test_fitted_p/exp_%s.png', num2str(exp_num)));
 end
