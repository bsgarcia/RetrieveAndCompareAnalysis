...
for sess = 1:2
    ...
    fit_params.cfcho = abs(data.cho-3);
    fit_params.out = data.out;
    fit_params.cfout = data.cfout;
    fit_params.con = data.con;
    fit_params.fit_cf = 1;
    fit_params.ntrials = 120;
    fit_params.models = learning_model;
    fit_params.nsub = size(data.cho, 1);
...
function [parameters,ll] = ...
    ...
    for sub = 1:fit_params.nsub
        ...
        waitbar(...
            ...
            w,...
            sprintf('%s%d%s%s', 'Fitting subject ', sub, ' in Exp. ', fit_params.sess)...
            );
