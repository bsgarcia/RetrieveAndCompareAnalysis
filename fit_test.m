...
for exp_name = filenames
    ...
    for j = 1:length(session)
        plot_learning_qvalues(d, idx, blue_color, exp_name, session(j), i);
    end
...
% --------------------------------------------------------------------%
function plot_learning_qvalues(d, idx, blue_color, exp_name, session, exp_num)
    i = 1;
    fit_folder = 'data/fit/';
    ...
    [alphas, ll] = runfit(...
        ...
        ntrials,...
        fit_folder,...
        sprintf('exp_%d_sess_%d_qvalues', exp_num, session));        
    ...
    p1 = plot(1:length(ev), mn2, 'LineWidth', 1.7, 'Color', blue_color);
    hold on
