...
scatterCorr(x, y1, orange_color, .6, 1, 70, 'white', 0);
xlabel('Heuristic score');
ylabel('-RT (ms)');
box off
set(gca, 'tickdir', 'out');

subplot(1, 2, 2)
scatterCorr(x, y2, green_color, .6, 1, 70, 'white', 0);
xlabel('Heuristic score');
ylabel('-RT (ms)');
box off
...
suptitle('Heuristic predicts -RT (ms)');
%y_mean = mean(ed')';
