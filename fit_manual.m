...
for exp_num = selected_exp
    ...
    fit_params.ncond = 4;
    fit_params.alpha_values = linspace(0, 1, 100);
...
function [parameters,ll] = ...
    ...
    for sub = 1:fit_params.nsub
        ...
        for model = fit_params.models
            ...
            %[throw, idx_max] = max(l1);
            where = find(l1 == max(l1));
            idx_max = where(randi(length(where)));
            parameters{model}(sub, :) = [1, fit_params.alpha_values(idx_max)];
            ll(model, sub) = l1(idx_max);
